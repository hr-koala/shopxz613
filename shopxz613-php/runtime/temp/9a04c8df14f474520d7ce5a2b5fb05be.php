<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:50:"./template/web/rainbow/mine\application_agent.html";i:1548744523;s:72:"D:\PHPTutorial\WWW\shopxz613-php\template\web\rainbow\public\header.html";i:1548744524;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="container" id="all">
    <div class="main">
        ...
        <div class="inp-level">
            ...
            <ul>
                <?php if(is_array($all_level) || $all_level instanceof \think\Collection || $all_level instanceof \think\Paginator): $i = 0; $__LIST__ = $all_level;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$level): $mod = ($i % 2 );++$i;?>
                    <li id="level<?php echo $level[level_id]; ?>"><?php echo $level[level_name]; ?></li>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </ul>
        </div>

        <?php if(is_array($all_level) || $all_level instanceof \think\Collection || $all_level instanceof \think\Paginator): $i = 0; $__LIST__ = $all_level;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$level): $mod = ($i % 2 );++$i;?>
            <div class="dealer-box" style="display: none;" id="level_conten<?php echo $level[level_id]; ?>">
                <div class="dealer-con">
                    <div class="dealer">
                        <div  class="dealer-img">
                            <img src="/template/web/rainbow/static/tonghaoran/images/proxy1.png">
                            <div class="dealer-img-txt">
                                <p class="p1"><?php echo $level[level_name]; ?></p>
                                <p class="p2">JINGXIAOSHANG</p>
                            </div>
                        </div>
                        <div class="dealer-txt">成为<?php echo $level[level_name]; ?></div>
                        <div class="dealer-num">¥ <?php echo $level[amount]; ?></div>
                    </div>
                    <div class="dealer-stats"><?php echo $level[describe]; ?></div>
                </div>

                <div class="taocan">产品列表</div>
                <div>
                    <?php if(is_array($upgrade_suite[upgrade_suite][$level[level_id]][goods]) || $upgrade_suite[upgrade_suite][$level[level_id]][goods] instanceof \think\Collection || $upgrade_suite[upgrade_suite][$level[level_id]][goods] instanceof \think\Paginator): $key = 0; $__LIST__ = $upgrade_suite[upgrade_suite][$level[level_id]][goods];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($key % 2 );++$key;?>
                        <div class="list">
                            <div class="box flex-row" data-num="1" data-price="895">
                                <div class="img"><img src="<?php echo $upgrade_suite[goods_info][$key-1][original_img]; ?>" alt=""></div>
                                <div class="right flex-column">
                                    <div class="name"><?php echo $upgrade_suite[goods_info][$key-1][goods_name]; ?><?php echo $key-1; ?></div>
                                    <!-- <div class="spec">规格:100ML规格:100ML规格:100ML规格:100ML规格:100ML规格:100ML规格:100ML</div> -->
                                    <div class="sbot flex-row">
                                        <div class="price">¥<?php echo $upgrade_suite[goods_info][$key-1][shop_price]; ?></div>
                                        <div class="flex-row" style="color: #1c1c1c;">X<?php echo $list; ?></div>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="inp-logo flex-row shop_show">
            ...
            <div class="logo-camera">
                <img src="/template/web/rainbow/static/tonghaoran/images/icon_camera.png">
            </div>
        </div>
        ...
        <div class="identity">
            ...
            <div class="identity1 identity1-photo">
                ...
                <div class="identity1-add">
                    <img src="/template/web/rainbow/static/images/icon_addimg.png">
    ...
    $(function(){
        ...
        $('.btn-submit').on('click', function(){
            ...
            if(flag){
                ...
                $.post("<?php echo U('cosmetology/Mine/data_submitting'); ?>", {invicode:invicode,username:username,userphone:userphone,useridcode:useridcode,wechatnumber:wechatnumber,usermailbox:usermailbox,storename:storename,storeintroduction:storeintroduction,shop_logo:shop_logo,grade:grade,identity:identity,arr:arr},function(datas){

                    var values = JSON.parse(datas);
                    // console.log(values);//125252526525645254
                    if(values['status'] == 1){
                        dialogFun(values['msg'],"<?php echo U('cosmetology/Mine/index'); ?>");
                    }else{
        ...
        function addPhoto() {
            var addDiv = '<div class="identity1-add"><img src="/template/web/rainbow/static/images/icon_addimg.png"/><input class="add-photo" type="file" accept="image/*" capture="camera"> </div>';
