<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:39:"./template/web/rainbow/index\index.html";i:1548744424;s:72:"D:\PHPTutorial\WWW\shopxz613-php\template\web\rainbow\public\header.html";i:1548744524;s:72:"D:\PHPTutorial\WWW\shopxz613-php\template\web\rainbow\public\footer.html";i:1548744524;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <?php if($shop_info != null): ?>
        <div class="m-shop">
            <div class="shop-avatar">
                <img src="<?php echo $shop_info[image_url]; ?>">
            </div>
            <div class="shop-name">
                <span class="shop-name1"><?php echo $shop_info['shop_name']; ?></span>
                <span><?php echo $shop_info['synopsis']; ?></span>
            </div>
        </div>
    <?php endif; ?>
    <div class="swiper-container">
        <div class="swiper-wrapper">
            <?php $pid =2;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1548748800 and end_time > 1548748800 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("5")->select();
...
foreach($result as $key=>$v):       
            ...
            <?php if(is_array($hot_area) || $hot_area instanceof \think\Collection || $hot_area instanceof \think\Paginator): $i = 0;$__LIST__ = is_array($hot_area) ? array_slice($hot_area,0,10, true) : $hot_area->slice(0,10, true); if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$area): $mod = ($i % 2 );++$i;?>
                <div class="goods-item" >
                    ...
                    <div class="gbot flex-row">
                        <div class="shop-coll">
                            <div class="icon icon-coll <?php echo in_array($area['goods_id'],$collect) == true ?'on':''; ?>" id="follow<?php echo $area['goods_id']; ?>"></div>
                            <div class="shop-coll-txt">收藏</div>
                        </div>

                        <div class="gleft" onclick="window.location.href='<?php echo U('cosmetology/Goods/details',['id'=>$area['goods_id']]); ?>'">
    ...
    <input type="hidden" class="grade" value="<?php echo $user_info==null?1:2; ?>">
...
$(function(){
    ...
    $('.icon-coll').click(function(){
        if(grade == 1){
            dialogFun('请先登录账号',"<?php echo U('cosmetology/User/login'); ?>");
        }
        var type = 0;
        ...
        $.post("<?php echo U('cosmetology/Goods/follow'); ?>", {follow:type,goods_id:goods_id},function(datas){
