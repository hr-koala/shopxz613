...
class PlaceOrder
{
    ...
    private function addOrderGoods()
    {
        ...
        $goodsArr = Db::name('goods')->where('goods_id', 'IN', $goods_ids)->getField('goods_id,cost_price,give_integral,is_agent');
        $orderGoodsAllData = [];
        foreach ($payList as $payKey => $payItem) {
            ...
            $orderGoodsData['is_agent'] = $goodsArr[$payItem['goods_id']]['is_agent'];
