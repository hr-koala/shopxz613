...
class Base extends Controller {
	...
	public function _initialize(){
		if(!empty($_COOKIE['mobile']) && !empty($_COOKIE['user_id'])){
		...
		$this->getLevel();
		$this->getFooter();
		$this->assign('goods_grade_price',C('goods_grade_price'));
		// $this->next_award();
	}
	...
	public function getLevel(){
		...
		$this->level_id = $user_info['level'];
		//所属店信息
		$shop = M('shop');
		if(empty($_COOKIE['shop_info']) && $user_info['shop_id'] != 0){
			$shop_info = $this->getShop($shop,'i.shop_id ='.$user_info['shop_id'].' AND shop_status=1 AND img.type=0');
			$this->assign('shop_info',$shop_info);
		}

		if(!empty($_COOKIE['shop_info'])){
			$shop_info_json = getDeCode($_COOKIE['shop_info'],'key');
			$this->shop_info = json_decode($shop_info_json,true);
		...
		$level_status = 0;
		if(!empty($level) && $level['level_id'] != 2){
	...
	public function getShop($shop,$where){
		$shop_info = $shop->alias('i')->join('newtp_shop_images img','img.shop_id = i.shop_id')->where($where)->find();
		return $shop_info;
	}
	...
	public function rotation_chart(){
		return $navigation = M('navigation')->where(['is_show'=>1,'position'=>'top'])->order('sort asc')->select();
	}
	...
	public function select_goods($where,$order){
		...
		$select_goods = Db::name('goods')->where($where)->field('goods_id,goods_name,shop_price,goods_remark,original_img,is_agent,explain,commodity_effect,distributor_price,lian_price,shopkeeper_price,market_generation_price,provincial_generation_price')->order($order)->cache(true,TPSHOP_CACHE_TIME)->select();
		return $select_goods;
	}
    ...
    //下发奖项
    public function next_award(){
    	$time = time();
    	$z_monthly_sales = M('z_monthly_sales');
    	$users_table = M('users');
    	//补货奖（统一发放）
    	$this->stock_award($time,$z_monthly_sales,$users_table);
    	//销售提成
    	$this->sales_commissions($time,$z_monthly_sales,$users_table);
    	//积分红利
    	$this->bonus_points($time,$z_monthly_sales,$users_table);

    }

    //销售提成
    public function sales_commissions($time,$z_monthly_sales,$users_table){
    	$sales_award = $this->sales_award();
    	$time = strtotime('20190201');//date('Y-m-d',$time); 测试完更改过来
    	dump($sales_award[0]['achievement']);
    	$where = [
    		's.two_total_purchase' => ['EGT',$sales_award[0]['achievement']],
    		's.sales_status' => 0,
    		's.settling_time' => ['ELT',date('Ymd',$time)],
    		'u.level' => ['in','10,11,12,13'],
    	];
    	$fields = 's.id,s.two_total_purchase,u.user_id,u.user_money,u.distribut_money';
    	$inte =  $z_monthly_sales->alias('s')->join('newtp_users u', 'u.user_id = s.user_id')->field($fields)->where($where)->select();
    	/*foreach ($inte as $k => $v) {
    		foreach ($sales_award as $key => $award) {
    			$v['']
    		}
    	}*/
    	dump($inte);
    ...
    //提成比例
    public function sales_award(){
    	return M('z_sales_award')->order('achievement asc')->select();
    }

    //积分红利
    public function bonus_points(){}

    //补货奖（统一发放）
    public function stock_award($time,$z_monthly_sales,$users_table){
    	//判断当前时间搓是否等于数据库中的时间搓
    	$now_time = strtotime('20190201');//date('Y-m-d',$time);
    	$where = [
    		's.stock_award_status' => 0,
    		's.settling_time' => ['ELT',date('Ymd',$time)],
    		'u.level' => ['in','10,11,12,13'],
    	];
    	$fields = 's.id,s.gross_purchases,s.three_gross_purchases,s.four_gross_purchases,s.five_gross_purchases,s.six_gross_purchases,s.seven_gross_purchases,s.settling_time,s.eight_gross_purchases,u.first_leader,u.user_id,u.user_money,u.distribut_money';
    	$inte =  $z_monthly_sales->alias('s')->join('newtp_users u', 'u.user_id = s.user_id')->field($fields)->where($where)->select();
    	$purchase_volume = $this->getConfig('purchase_volume');
    	if(!empty($inte)){
    		$get_award_user = [];
    		foreach ($inte as $ke => $va) {
    			foreach ($inte as $k => $v) {
    				if($v['first_leader'] == $va['user_id']){
    					if($v['gross_purchases'] > $purchase_volume || $v['gross_purchases'] == $purchase_volume){
    						$get_award_user[$ke]['id'] = $va['id'];
    						$get_award_user[$ke]['user_id'] = $va['user_id'];
    						$get_award_user[$ke]['user_money'] = $va['user_money'];
    						$get_award_user[$ke]['distribut_money'] = $va['distribut_money'];
    						$get_award_user[$ke]['settling_time'] = $va['settling_time'];
    						$get_award_user[$ke]['award_num'] += 1;
    						$get_award_user[$ke]['four_gross_purchases'] = $va['four_gross_purchases'];
    						$get_award_user[$ke]['five_gross_purchases'] = $va['five_gross_purchases'];
    						$get_award_user[$ke]['six_gross_purchases'] = $va['six_gross_purchases'];
    						$get_award_user[$ke]['seven_gross_purchases'] = $va['seven_gross_purchases'];
    						$get_award_user[$ke]['eight_gross_purchases'] = $va['eight_gross_purchases'];
    					}
    				}
    			}
    			if($va['gross_purchases'] > $purchase_volume || $va['gross_purchases'] == $purchase_volume){
    				$get_award_user[$ke]['id'] = $va['id'];
    				$get_award_user[$ke]['user_id'] = $va['user_id'];
    				$get_award_user[$ke]['user_money'] = $va['user_money'];
    				$get_award_user[$ke]['distribut_money'] = $va['distribut_money'];
    				$get_award_user[$ke]['settling_time'] = $va['settling_time'];
    				$get_award_user[$ke]['own_status'] = 1;
    				$get_award_user[$ke]['three_gross_purchases'] = $va['three_gross_purchases'];
    			}
    		}
    		$this->Lower_purchase_award($users_table,$z_monthly_sales,$get_award_user);
    	}
    }

    //下发进货奖
    public function Lower_purchase_award($users_table,$z_monthly_sales,$get_award_user){
		$purchase_ratio = $this->getConfig('purchase_ratio');
		$count = count($get_award_user);
		for ($i=0; $i < $count; $i++) {
			$value = 0;
			if(!empty($get_award_user[$i]['award_num']) && $get_award_user[$i]['award_num'] != 0){
				if($get_award_user[$i]['award_num'] == 1){
					$value = $get_award_user[$i]['four_gross_purchases'];
				}elseif($get_award_user[$i]['award_num'] == 2){
					$value = ($get_award_user[$i]['four_gross_purchases']+$get_award_user[$i]['five_gross_purchases']);
				}elseif($get_award_user[$i]['award_num'] == 3){
					$value = ($get_award_user[$i]['four_gross_purchases']+$get_award_user[$i]['five_gross_purchases']+$get_award_user[$i]['six_gross_purchases']);
				}elseif($get_award_user[$i]['award_num'] == 4){
					$value = ($get_award_user[$i]['four_gross_purchases']+$get_award_user[$i]['five_gross_purchases']+$get_award_user[$i]['six_gross_purchases']+$get_award_user[$i]['seven_gross_purchases']);
				}elseif($get_award_user[$i]['award_num'] == 5){
					$value = ($get_award_user[$i]['four_gross_purchases']+$get_award_user[$i]['five_gross_purchases']+$get_award_user[$i]['six_gross_purchases']+$get_award_user[$i]['seven_gross_purchases']+$get_award_user[$i]['eight_gross_purchases']);
				}
			}
			if(!empty($get_award_user[$i]['own_status']) && $get_award_user[$i]['own_status'] == 1){
				$value = ($value+$get_award_user[$i]['three_gross_purchases'])*($purchase_ratio/100);
			}
			$save['user_money'] = $value+$get_award_user[$i]['user_money'];
			$save['distribut_money'] = $value+$get_award_user[$i]['distribut_money'];
			$where['user_id'] = $get_award_user[$i]['user_id'];
			$res = $users_table->where($where)->save($save);
			if($res && $value > 0){
				$z_monthly_sales->where($where)->where(['id'=>$get_award_user[$i]['id']])->save(['stock_award_status'=>2,'stock_award_money'=>$value]);
				$this->bonus_record($get_award_user[$i]['user_id'],$value,'每月进货奖结算',0,0,0,3,strtotime($get_award_user[$i]['settling_time']));
			}
		}
    }

    public function fenyongceshi(){
    	// dump(C('gross_purchases_name')[2]);die;
    	$this->uid = '52';
    	$order = M('order');
    	$order_info = $order->where(['user_id'=>$this->uid,'order_sn'=>'201901041531563882'])->find();
    	$this->distribution($order_info);
    }

    //问题：
	//1、礼包可以重复买吗？（所有的礼包类型只能买一次 A，还是所有的礼包可以各买一次 B）
		//B1、如果所有升级礼包可以各买一次，那升级店主再去买比店主低层级的经销商升级礼包可以吗？
		//B2、如果所有升级礼包可以各买一次，那推广奖买一种礼包就算一次？	

	//判断订单类型 1:购买商品 2:购买礼包 3:进货
	  //1:购买商品->判断用户身份
		  //a:普通会员/经销商的价格购买商品
		  //b:联创或联创以上，b1:购买价格不一样，b2:销售提成，b3:积分红利
	  //2:购买礼包
		 //判断身份：没有身份
			//判断购买礼包晋升层级
			  	//1：经销商->升级
			  	//2：联创或联创以上：推广奖、销售提成、升级、生成推荐码、积分分红
		 //判断身份：有身份
			//判断是否为第一次购买此层级礼包：是----------------------------------------------------------未完待续
			//判断是否为第一次购买此层级礼包：不是--------------------------------------------------------未完待续
	  //3:进货
		//判断身份：a经销商->没有奖励
		//判断身份：b联创/c店主/d省代/e市代->补货奖、销售提成、积分红利

	//--------------------------------------------------------------------------------------------------
	//A 推广奖金（实时返）
		// 1、	联创级别以上享有此奖金，（普通会员与经销商没有）
		// 2、	直推8%，间推4%（1后台设置推广奖金比例）
		// 判断是否有上级，有->返给上级8%
		// 判断上级的上级是否有上级，->有->返给上级的上级4%
		// 3、	推广奖金记录

	//B 补货奖：（按月发）
		//1、记录自己当月进货是否满足2000元
		//2、记录上级的直属下级是第几次满足2000元
		//3、记录上三至八层当月补货总业绩
		//4、记录自己当月补货总业绩
		//5、记录团队总业绩无上上级递归
		//6、记录月头结算的时间戳，不同次数获得不同奖金

	//C 销售提成（按月结算）
		//1、记录自己本月购买总金额（算首单与不算首单两个字段）
		//2、记录（去除每一层的首单）团队总业绩无限递归计算上级
		//判断是否存在上级：存在并且满足销售额（记录每个人当月个人业绩（不算自己的首单）+团队业绩（去除每一层的首单））->（找出满足相对应的销售额）对应的提成比例 * 购买金额；

	//D 积分分红（按月结算）（满300W，获得1%加权分红）
		//1、记录当月团队无限递归上级总业绩
		//当月无限直属下级销售额满300W，获得1%（4后台设置积分分红比例）->判断此时是否满足300W->满足无限递归返给上级

	//E 升级机制
		//1、记录团队无限递归上级联创身份
		//判断当前是否购买（联合创始人/店主/市代 /省代）升级礼包->存在其中一个
		// ->当前用户生成邀请码
		// ->（如是联合创始人无限递归上级存储身份）存储上级的直属下级身份个数->判断上级是否满足（下线身份会员个数）->满足->上级升级->（如是联合创始人无限递归上级是否能升级）

	//联创：推广奖、补货奖、销售提成、判断是否能晋级、生成推荐码
	//店主：推广奖、补货奖、销售提成、全套仪器与产品
	//市代：推广奖、补货奖、销售提成、城市业绩的1%
	//省代：推广奖、补货奖、销售提成、城市业绩的1%
...
//
    ...
    //分佣开始
	public function distribution($order_info){
		$users_table = M('users');
		$superior = '';
		//获取当前用户身份
		$users = $this->identification($users_table,$order_info['user_id']);
		//判断当前用户是否存在上两级
		if(!empty($users['info']['invitation_code_others']) && $users['info']['invitation_code_others'] != 0){
			$superior = $this->is_exist_superior($users_table,$users['info']['invitation_code_others']);
		}
		//判断订单类型 1:购买商品 2:购买礼包 3:进货
		if($order_info['order_type'] == 1 && !empty($users['level']) && in_array($users['level']['level_id'], ['10','11','12','13'])){//1:购买商品
			//判断用户身份-a:普通会员/经销商的价格购买商品(无操作)、b:联创或联创以上，b1:购买价格不一样，b2:销售提成，b3:积分红利
		}elseif($order_info['order_type'] == 2){//2:购买礼包
			
			if(empty($users['level'])){//判断身份：没有身份
				//升级、生成推荐码
				$this->upgrade($users_table,$users['info']['user_id'],$order_info['upgrade_type'],$users['info']['level'],$superior);
				//推广奖
				$this->promotion_award($users_table,$users['info']['user_id'],$order_info['total_amount'],$order_info['order_sn'],$order_info['order_id'],$superior);

			}elseif(!empty($users['level']) && in_array($users['level']['level_id'], ['10','11','12','13'])){
				if($order_info['upgrade_type'] > $users['info']['level']){
					//判断身份：有身份(补货奖、销售提成、积分分红)
					$this->purchase_award($users_table,$order_info['user_id'],$order_info['total_amount'],$superior);//补货奖参数
				}
				//高于自己需要晋级
				if($order_info['upgrade_type'] > $users['info']['level']){
					$this->upgrade($users_table,$users['info']['user_id'],$order_info['upgrade_type'],$users['info']['level'],$superior,1);
				}
			}
		
		}elseif($order_info['order_type'] == 3 && !empty($users['level']) && in_array($users['level']['level_id'], ['10','11','12','13'])){//3:进货
			//判断身份：a经销商->没有奖励(无操作)、b联创/c店主/d省代/e市代->补货奖、销售提成、积分红利
			$this->purchase_award($users_table,$order_info['user_id'],$order_info['total_amount'],$superior);//补货奖参数
		}

		//记录到每月总销售额是会员连存自己，不是会员不存自己
		//是否达到销售业绩提成
		$this->monthly_sales($users_table,$users['info'],$order_info['total_amount'],$superior);
		//
		//积分分红 满300W，团队总业绩*1%加权分红
		//判断当前用户直属下级销售额是否达到300W，如达到，无限上级循环获得1个积分
		//（记录每个人当月个人业绩+团队业绩）->（找出满足相对应的销售额）对应的提成比例 * 购买金额；
		//
		//
		//每个城市每个每月的总业绩Urban performance
		//
		//

		/*//销售提成、积分红利
		if(!empty($users['level']) && in_array($users['level']['level_id'], ['10','11','12','13'])){
			//销售提成
			//1、记录自己本月购买总金额
			//2、记录团队总业绩无限递归计算上级
			//判断是否存在上级：存在并且满足销售额（记录每个人当月个人业绩+团队业绩）->（找出满足相对应的销售额）对应的提成比例 * 购买金额；
			//销售提成->未完待续-------------
			//
			//
			//积分红利
		...
		//城市总业绩的1%
		if(!empty($users['level']) && in_array($users['level']['level_id'], ['12','13'])){}*/
	}

	//记录每个月用户的销售额
	public function monthly_sales($users_table,$user_info,$total_amount,$superior){
		$z_monthly_sales = M('z_monthly_sales');
		//记录到每月总销售额是会员连存自己，不是会员不存自己
		$data = [
		...
		$own_integral_status = 0;
		if(in_array($user_info['level'], ['10','11','12','13'])){
			$user_sales = $z_monthly_sales->where($data)->find();
			if(!empty($user_sales)){
				if($user_info['level'] == '12' || $user_info['level'] == '13'){
					$save['urban_performance'] = '-1';
				}
				if(($user_sales['two_total_purchase'] > 3000000 && $user_sales['integral_status'] == 0) || ($superior_save['two_total_purchase'] == 3000000 && $user_sales['integral_status'] == 0)){
					$own_integral_status = $save['integral_status'] = 1;
				}
				$save['own_total_purchase'] = $user_sales['own_total_purchase']+$total_amount;
				$z_monthly_sales->where($data)->save($save);
			}else{
				if($user_info['level'] == '12' || $user_info['level'] == '13'){
					$data['urban_performance'] = '-1';
				}
				$data['own_total_purchase'] = $total_amount;
				$data['settling_time'] = date('Ymd',mktime(0,0,0,+($data['month']+1),1,$data['year'])); 
				$z_monthly_sales->add($data);
			}
		}
		//计入上级及无限上级当月总业绩
		if($superior != ''){
			//计入上级的直属下级销售业绩
			$superior_data = [
				'user_id' => $superior['one']['user_id'],
				'month' => $data['month'],
				'year' => $data['year']
			];
			$superior_user_sales = $z_monthly_sales->where($superior_data)->find();
			$integral_num = 0;
			if(!empty($superior_user_sales)){
				if($superior['one']['level'] == '12' || $superior['one']['level'] == '13'){
					$superior_save['urban_performance'] = '-1';
				}
				$superior_save['two_total_purchase'] = $superior_user_sales['two_total_purchase']+$total_amount;
				//判断当前上级是否满300W,满300W计入无限上级
				if(($superior_save['two_total_purchase'] > 3000000 && $superior_user_sales['integral_status'] == 0) || ($superior_save['two_total_purchase'] == 3000000 && $superior_user_sales['integral_status'] == 0)){
					$integral_num = $superior_save['integral_status'] = 1;
				}
				if($own_integral_status == 1 && $superior_user_sales['integral_status'] == 0){
					$integral_num = $superior_save['integral_status'] = 1;
					$superior_save['integral_num'] = $superior_user_sales['integral_num']+1;
				}
				$superior_save['team_total_purchase'] = $superior_user_sales['team_total_purchase']+$total_amount;
				$z_monthly_sales->where($superior_data)->save($superior_save);
			}else{
				if($superior['one']['level'] == '12' || $superior['one']['level'] == '13'){
					$superior_data['urban_performance'] = '-1';
				}
				if($own_integral_status == 1){
					$integral_num = $superior_data['integral_status'] = $superior_data['integral_num'] = 1;
				}
				$superior_data['team_total_purchase'] = $superior_data['two_total_purchase'] = $total_amount;
				$superior_data['settling_time'] = date('Ymd',mktime(0,0,0,+($data['month']+1),1,$data['year'])); 
				$z_monthly_sales->add($superior_data);
			}
			//计入无限上级当月总业绩
			if(!empty($superior['two'])){
				$this->superior_monthly_sales($users_table,$z_monthly_sales,$superior['two'],$total_amount,$data['year'],$data['month'],$integral_num);
			}
		}
		//计入城市当月总业绩
		//当前月份字段 strtolower(date('F'))
	}

	//计入上级及无限上级当月总业绩
	public function superior_monthly_sales($users_table,$z_monthly_sales,$superior,$total_amount,$year,$month,$superior_integral_num){
		//计入上级的直属下级销售业绩
		$superior_data = [
		...
		$integral_num = 0;
		if(!empty($superior_user_sales)){
			if($superior['level'] == '12' || $superior['level'] == '13'){
				$superior_save['urban_performance'] = '-1';
			}
			//判断当前上级是否满300W,满300W计入无限上级
			if(($superior_user_sales['two_total_purchase'] > 3000000 && $superior_user_sales['integral_status'] == 0) || ($superior_user_sales['two_total_purchase'] == 3000000 && $superior_user_sales['integral_status'] == 0)){
				$integral_num = $superior_save['integral_status'] = 1;
			}
			if($superior_integral_num == 1 && $superior_user_sales['integral_status'] == 0){
				$integral_num = $superior_save['integral_status'] = 1;
				$superior_save['integral_num'] = $superior_user_sales['integral_num']+1;
			}
			$superior_save['team_total_purchase'] = $superior_user_sales['team_total_purchase']+$total_amount;
			...
			if($superior_integral_num == 1){
				$integral_num = $superior_data['integral_status'] = $superior_data['integral_num'] = 1;
			}
			$superior_data['team_total_purchase'] = $total_amount;
			$superior_data['settling_time'] = date('Ymd',mktime(0,0,0,+($data['month']+1),1,$data['year'])); 
			$z_monthly_sales->add($superior_data);
		}
		//计入无限上级当月总业绩
		$user = $users_table->where(['user_id'=>$superior['first_leader']])->find();
		if(!empty($user)){
			$this->superior_monthly_sales($users_table,$z_monthly_sales,$user,$total_amount,$year,$month,$integral_num);
		}
	}

	//积分红利
	public function bonus_bonus(){}

	//补货奖
	public function purchase_award($users_table,$user_id,$total_amount,$superior){
		//B 补货奖：（按月发）
			//1、记录自己当月进货总额（需要判断当月进货是否满足2000元）
			//2、记录上级的直属下级当月进货总额
			//3、记录上三至八层当月补货总业绩
			//5、记录团队总业绩无限上级递归
			//6、记录月头结算的时间戳，不同次数获得不同奖金
        ...
        $inte = $z_monthly_sales->where($data)->find();
        if(empty($inte)){
        	$data['settling_time'] = date('Ymd',mktime(0,0,0,+($month+1),1,$year)); 
        	$data['total_consumption'] = $data['gross_purchases'] = $total_amount;
        	$z_monthly_sales->add($data);
        }else{
        	$save['total_consumption'] = $inte['total_consumption']+$total_amount;
        	$save['gross_purchases'] = $inte['gross_purchases']+$total_amount;
        	$z_monthly_sales->where($data)->save($save);
        }

        //上级的总业绩记录
        if($superior != ''){
        	$this->total_performance($users_table,$z_monthly_sales,$year,$month,$superior['one'],$total_amount,1);
        }
	}

	//总业绩记录
	public function total_performance($users_table,$z_monthly_sales,$year,$month,$superior,$total_amount,$i){
		if($i < 8 || $i == 8){
			$name = C('gross_purchases_name')[$i];
			$data = [
				'year' => $year,
				'month' => $month,
				'user_id' => $superior['user_id'],
	        ...
	        if(empty($inte)){
	        	$data['settling_time'] = date('Ymd',mktime(0,0,0,+($month+1),1,$year)); 
	        	$data[$name] = $data['all_gross_purchases'] = $total_amount;
	        	$z_monthly_sales->add($data);
	        }else{
	        	$save[$name] = $inte[$name]+$total_amount;
	        	$save['all_gross_purchases'] = $inte['all_gross_purchases']+$total_amount;
	        	$z_monthly_sales->where($data)->save($save);
	        }
	        $user = $users_table->where(['user_id'=>$superior['first_leader']])->find();
	        if(!empty($user)){
	        	$this->total_performance($users_table,$z_monthly_sales,$year,$month,$user,$total_amount,$i++);
	        }
	    }
	}

	/*public function gross_purchases_name($i){
		switch ($i) {
			case 1:
				$name = 'superior_gross_purchases';
				break;
			case 2:
				$name = 'two_gross_purchases';
				break;
			case 3:
				$name = 'three_gross_purchases';
				break;
			case 4:
				$name = 'four_gross_purchases';
				break;
			case 5:
				$name = 'five_gross_purchases';
				break;
			case 6:
				$name = 'six_gross_purchases';
				break;
			case 7:
				$name = 'seven_gross_purchases';
				break;
			default:
				$name = 'eight_gross_purchases';
				break;
		}
		return $name;
	}*/
	...
	// 推广奖金（实时返）
	// 1、判断上级是否为联创级别以上，享有此奖金，（普通会员与经销商没有）
	// 2、直推8%，间推4%（1后台设置推广奖金比例）
	// 判断是否有上级，有->返给上级8%
	// 判断上级的上级是否有上级，->有->返给上级的上级4%
	// 3、	推广奖金记录
	public function promotion_award($users_table,$user_id,$total_amount,$order_sn,$order_id,$superior){
		if(in_array($superior['one']['level'], ['10','11','12','13']) && $superior != ''){
			//直推奖金比例
			$straight_award = $this->getConfig('straight_award');
			$award = $total_amount*($straight_award/100);
			$one_where = 'user_id='.$superior['one']['user_id'];
			$users_table->where($one_where)->setInc('user_money',$award);
			$users_table->where($one_where)->setInc('distribut_money',$award);
			//奖金记录
			$this->bonus_record($superior['one']['user_id'],$award,'下级购买升级礼包获得推广奖',$order_sn,$order_id,$user_id,1);
		}
		if(in_array($superior['two']['level'], ['10','11','12','13']) && $superior != ''){
			//间推奖金比例
			$push_between = $this->getConfig('push_between');
			$push_between_award = $total_amount*($push_between/100);
			$two_where = 'user_id='.$superior['two']['user_id'];
			$users_table->where($two_where)->setInc('user_money',$push_between_award);
			$users_table->where($two_where)->setInc('distribut_money',$push_between_award);
			//奖金记录
			$this->bonus_record($superior['two']['user_id'],$push_between_award,'下级的下级购买升级礼包获得间推奖',$order_sn,$order_id,$user_id,2);
		}
	}

	//奖金记录
	public function bonus_record($user_id,$user_money,$desc,$order_sn,$order_id,$subordinate_user_id,$type,$time=''){
		if($time == ''){ $time = time();}
		$data = [
			'user_id' => $user_id,
	...
	//升级  
	public function upgrade($users_table,$user_id,$upgrade_type,$level,$superior,$type=''){
		$z_upgrade_log = M('z_upgrade_log');
		if(in_array($level, ['10','11','12','13']) && $type == ''){//生成推荐码
			$save['invitation_code_own'] = getyqcodes();
		}
		//用户升级
		$save['level'] = $upgrade_type;
		$res = $users_table->where(['user_id'=>$user_id])->save($save);
		//上级的上级是否升级
		if($superior != '' && in_array($superior['one']['level'], ['10','11','12','13']) && in_array($level, ['10','11','12','13'])){
			$get_all_level = $this->get_all_level();
			switch ($level) {
				case 10:
					$superior_save['liam_num'] = $superior['one']['liam_num']+1;//联创人数加1
					$superior_save['team_liam_num'] = $superior['one']['team_liam_num']+1;//团队联创人数加1
					//无限循环联创人数加1
					if(in_array($superior['two']['level'], ['10','11','12','13'])){
						$this->loop_superior($users_table,$z_upgrade_log,$superior['two'],$get_all_level);
					}
					//判断上级是否能升级
					if($superior_save['liam_num'] == $get_all_level[11]['lower_rank'] && $superior['one']['team_liam_num'] == $get_all_level[11]['team'] && $superior['one']['level'] == 10){
						$superior_save['level'] = 11;
					}
					break;
				case 11:
					$superior_save['shopkeeper_num'] = $superior['one']['shopkeeper_num']+1;//店主人数加1
					//判断上级是否能升级
					if($superior_save['shopkeeper_num'] == $get_all_level[12]['lower_rank'] && in_array($superior['one']['level'], ['10','11'])){
						$superior_save['level'] = 12;
					}
					break;
				case 12:
					$superior_save['city_agent_num'] = $superior['one']['city_agent_num']+1;//市代人数加1
					//判断上级是否能升级
					if($superior_save['city_agent_num'] == $get_all_level[13]['lower_rank'] && in_array($superior['one']['level'], ['10','11','12'])){
						$superior_save['level'] = 13;
					}
					break;
				default:
					$superior_save['provincial_agent_num'] = $superior['one']['provincial_agent_num']+1;//省代人数加1
					break;
			}
			if(!empty($superior_save)){
				$users_table->where(['user_id'=>$superior['one']['user_id']])->save($superior_save);
			}
			if(!empty($superior_save['level'])){
				//升级记录
				$z_upgrade_log->add(['user_id'=>$superior['one']['user_id'],'original_grade'=>$superior['one']['level'],'current_grade'=>$superior_save['level'],'time'=>time()]);
				if(!empty($superior['two']) && in_array($superior['two']['level'], ['10','11','12','13'])){
					$this->loop_superior_two($users_table,$z_upgrade_log,$superior['two'],$get_all_level,$superior_save['level']);
				}
			}
		}
		if($res){
			//升级记录
			$z_upgrade_log->add(['user_id'=>$user_id,'original_grade'=>$level,'current_grade'=>$upgrade_type,'time'=>time()]);
		}
	}

	//无限循环上级升级
	public function loop_superior($users_table,$z_upgrade_log,$superior,$get_all_level){
		$res = $users_table->where('user_id='.$superior['user_id'])->setInc('team_liam_num');//团队联创人数加1
		$team_liam_num = $superior['team_liam_num']+1;
		//上级的上级信息
		$user = $users_table->where(['user_id'=>$superior['first_leader']])->find();
		//判断当前上级是否能升级
		if($superior['level'] == 10 && $get_all_level[11]['lower_rank'] == $superior['liam_num'] && $team_liam_num == $get_all_level[11]['team']){
			$res = $users_table->where(['user_id'=>$superior['user_id']])->save(['level'=>11]);
			if($res){
				$z_upgrade_log->add(['user_id'=>$superior['user_id'],'original_grade'=>$superior['level'],'current_grade'=>11,'time'=>time()]);
				if(!empty($user) && in_array($user['level'], ['10','11','12','13'])){
					$this->loop_superior_two($users_table,$z_upgrade_log,$user,$get_all_level,11);
				}
			}
		}
		if(!empty($user) && in_array($user['level'], ['10','11','12','13'])){
			$this->loop_superior($users_table,$z_upgrade_log,$user,$get_all_level);
		}
	}

	public function loop_superior_two($users_table,$z_upgrade_log,$superior,$get_all_level,$level){
		if($level == 11){
			$save['shopkeeper_num'] = $superior['shopkeeper_num']+1;//店主人数加1
			if($save['shopkeeper_num'] == $get_all_level[12]['lower_rank']){
				$save['level'] = 12;
			}
		}elseif($level == 12){
			$save['city_agent_num'] = $superior['city_agent_num']+1;//店主人数加1
			if($save['city_agent_num'] == $get_all_level[13]['lower_rank']){
				$save['level'] = 13;
			}
		}elseif($level == 13){
			$save['provincial_agent_num'] = $superior['provincial_agent_num']+1;//店主人数加1
		}
		if(!empty($save)){
			$users_table->where(['user_id'=>$superior['user_id']])->save($save);
		}
		if(!empty($save['level'])){
			$z_upgrade_log->add(['user_id'=>$superior['user_id'],'original_grade'=>$superior['level'],'current_grade'=>$save['level'],'time'=>time()]);
			$user = $users_table->where(['user_id'=>$superior['first_leader']])->find();
			if(!empty($user) && in_array($user['level'], ['10','11','12','13'])){
				$this->loop_superior_two($users_table,$z_upgrade_log,$user,$get_all_level,$save['level']);
			}
		}
	}

	//判断用户身份
	public function identification($users_table,$user_id){
		$users['info'] = $users_table->where(['user_id'=>$user_id])->find();
		$user_level = $this->get_current_level(['level_id'=>$users['info']['level']]);
		if(!empty($user_level)){
			$users['level'] = $user_level;
		}
		return $users;
	}

	//当前用户层级 
	public function get_current_level($where){
		return $user_level = M('user_level')->where($where)->find();
	}

	//判断当前用户是否存在上两级
	public function is_exist_superior($users_table,$invitation_code_others){
		$one_superior = $users_table->where(['invitation_code_own'=>$invitation_code_others,'is_lock'=>0])->find();
		if(!empty($one_superior)){
			$superior['one'] = $one_superior;
			if(!empty($one_superior['invitation_code_others']) && $one_superior['invitation_code_others'] != 0){
				$two_superior = $users_table->where(['invitation_code_own'=>$one_superior['invitation_code_others'],'is_lock'=>0])->find();
			}
			if(!empty($two_superior)){
				$superior['two'] = $two_superior;
			}
			return $superior;
		}
	}
	...
	public function get_all_level(){
		return $all_level = M('user_level')->order('level_id asc')->getField('level_id,level_name,amount,describe,lower_rank,team');
	}
	
	//地址
    public function getAddress(){
        $user_address = Db::name('UserAddress');
        $address_list = $user_address->where(['user_id' => $this->user_id])->order('is_default desc')->select();
        if ($address_list) {
            $area_id = array();
        ...
        if ((count($address_list) > 0) && ($c == 0)){ // 如果没有设置默认收货地址, 则第一条设置为默认收货地址
            ...
            $user_address->where(['user_id'=>$this->user_id,'address_id'=>$address_list[0]['address_id']])->save(['is_default'=>1]);
        }
        // dump($address_list[0]);
        $this->assign('address', $address_list[0]);
    }

	//备份
	// 生成订单时记录订单类型 1:购买商品 2:购买礼包 3:进货
...
public function aa(){
	$res = $this->Get_ID(6);//100000000000000001
	dump($res);
}
