...
class User extends Base {
    ...
    public function ajaxindex(){
        ...
        I('user_id')?$condition['first_leader'] = I('user_id') : false;
        I('registration_type')?$condition['registration_type'] = ['neq',0] : false;
        ...
        $users = M('users');
        $bank_user = $users->where(['registration_type'=>['in','1,2'],'is_lock'=>0,'first_leader'=>['neq',0]])->getField('user_id,first_leader');
        $bank_user = array_flip(array_unique($bank_user));
        ...
        // dump($bank_user);die;
        $this->assign('bank_user',$bank_user);
    ...
    public function subordinate(){
        $users = M('users');
        $user_info = $users->where('user_id='.I('id'))->find();
    ...
    public function account_log(){
        ...
        $lists  = M('account_log')->where(array('user_id'=>$user_id))->order('change_time desc')->select();
...
/*
        ...
        $log_info = $account_log->where(['log_id'=>$log_id])->find();
        if(empty($log_info) || $log_info['type'] != 6 && $log_info['type'] != 7){
            echo json_encode(['status'=>0,'msg'=>'返款信息不存在']);exit;
        }
        if($log_info['type'] == 6 && $log_info['sales_award'] > 0){//销售提成
            $save['user_money'] = $log_info['sales_award'];
            $save['sales_award'] = 0;
        }
        if($log_info['type'] == 7 && $log_info['bonus'] > 0){//销售提成
            $save['user_money'] = $log_info['bonus'];
            $save['bonus'] = 0;
        }
        if(!empty($save)){
            $save['refunds_time'] = time();
            $res = $account_log->where(['log_id'=>$log_id])->save($save);

            $users = M('users');
            $user_info = $users->where(['user_id'=>$log_info['user_id']])->find();
            $user_save['user_money'] = $user_info['user_money']+$save['user_money'];
            $user_res = $users->where(['user_id'=>$log_info['user_id']])->save($user_save);
        }
        if($user_res || $res){
            echo json_encode(['status'=>1,'msg'=>'返款成功']);exit;
            // $this->ajaxReturn(['status'=>1,'msg'=>'返款成功']);exit;
        }else{
            echo json_encode(['status'=>0,'msg'=>'返款失败']);exit;
            // $this->ajaxReturn(['status'=>0,'msg'=>'返款失败']);exit;
        }
    }
    ...
    public function account_edit(){
        ...
        $user = M('users')->field('user_id,user_money,frozen_money,pay_points,is_lock,bank_note')->where('user_id',$user_id)->find();
        if(IS_POST){
            $desc = I('post.desc');
            if(!$desc)
            ...
            $bank_note = I('post.bank_note/d');
            $bank_note =  $p_op_type ? $bank_note : 0-$bank_note;
            if(($bank_note+$user['bank_note'])<0 ){
            ...
            if(accountLog($user_id,$user_money,$bank_note,$desc,0))
            {
