...
class CartLogic extends Model
{
    ...
    public function setGoodsModel($goods_id)
    {
        if ($goods_id > 0) {
            ...
            $goods_table = M('goods');
            $this->goods = $goods_table->where('goods_id='.$goods_id)->find();
        }
    }
    ...
    public function setUserId($user_id)
    {
        ...
        $users = M('users');
        $this->level_id = $users->where('user_id='.$user_id)->getField('level');
    }
    ...
    public function buyNow()
    {
        ...
        $buyGoods = [
            ...
            'member_goods_price' => $this->goods[C('goods_grade_price')[$this->level_id]],
        ...
        if (empty($this->specGoodsPrice)) {
            ...
            $goods_grade_price = C('goods_grade_price')[$this->level_id];
            if($this->level_id == 1){
                $goods_grade_price = 'price';
            }
            $buyGoods['member_goods_price'] = $this->specGoodsPrice[$goods_grade_price];
        ...
        if ($goodsPromFactory->checkPromType($prom_type)) {
            ...
            if ($this->goods['prom_type'] == 0) {
                if (!empty($this->goods['price_ladder'])) {
                    ...
                    $buyGoods['member_goods_price'] = round($buyGoods['member_goods_price'] * $discount, 2);
                    $buyGoods['goods_price'] = round($buyGoods['goods_price'] * $discount, 2);
                }
            }
        }
        $cart = new Cart();
    ...
    {
        if (empty($this->specGoodsPrice)) {
            $price = $this->goods[C('goods_grade_price')[$this->level_id]];
            $original_price = $this->goods['shop_price'];
            $store_count = $this->goods['store_count'];
        } else {
            ...
            $price = $this->specGoodsPrice[$goods_grade_price];
            $original_price = $this->specGoodsPrice['price'];
            $store_count = $this->specGoodsPrice['store_count'];
        }
        // 查询购物车是否已经存在这商品
        ...
        if ($userCartGoods) {
            ...
            if (!empty($this->goods['price_ladder'])) {
                $goodsLogic = new GoodsLogic();
                $price_ladder = $this->goods['price_ladder'];
                $price = $goodsLogic->getGoodsPriceByLadder($userWantGoodsNum, $this->goods[C('goods_grade_price')[$this->level_id]], $price_ladder);
                $original_price = $goodsLogic->getGoodsPriceByLadder($userWantGoodsNum, $this->goods['shop_price'], $price_ladder);
            }
            if ($userWantGoodsNum > 200) {
                $userWantGoodsNum = 200;
            }
            ...
            $cartResult = $userCartGoods->save(['goods_num' => $userWantGoodsNum, 'goods_price' => $original_price, 'member_goods_price' => $price]);
        } else {
            //如果该商品没有存在购物车
            if ($this->goodsBuyNum > $store_count) {
            ...
            if (!empty($this->goods['price_ladder'])) {
                $goodsLogic = new GoodsLogic();
                $price_ladder = $this->goods['price_ladder'];
                $price = $goodsLogic->getGoodsPriceByLadder($this->goodsBuyNum, $this->goods[C('goods_grade_price')[$this->level_id]], $price_ladder);
                $original_price = $goodsLogic->getGoodsPriceByLadder($this->goodsBuyNum, $this->goods['shop_price'], $price_ladder);
            }
    ...
    private function addFlashSaleCart()
    {
        ...
        if ($userCartGoods) {
            ...
            if ($this->specGoodsPrice) {
                ...
                $cartAddFlashSaleData['goods_price'] = $this->specGoodsPrice[$goods_grade_price];   // 规格价
                $cartAddFlashSaleData['prom_id'] = $this->specGoodsPrice['prom_id']; // 活动id
            } else {
                $cartAddFlashSaleData['goods_price'] = $this->goods[C('goods_grade_price')[$this->level_id]];   // 原价
    ...
    private function addPromGoodsCart()
    {
        ...
        if ($this->specGoodsPrice) {
            ...
            $priceBefore = $this->specGoodsPrice[$goods_grade_price];
            $storeCount = $this->specGoodsPrice['store_count'];
        } else {
            $priceBefore = $this->goods[C('goods_grade_price')[$this->level_id]];
