...
function recursion($data, $id) {
    $list = array();
    foreach($data as $v) {
        if($v['invitation_code_others'] == $id) {
            $v['son'] = recursion($data, $v['invitation_code_own']);
            if(empty($v['son'])) {
                unset($v['son']);
            }
            array_push($list, $v);
