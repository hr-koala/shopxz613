...
class Report extends Base
{
  ...
  /**
    ...
    public function monthly_report(){
        ...
        $smsTpls = $z_monthly_report->order('year DESC,month asc')->select();
        $this->assign('smsTplList',$smsTpls);
        return $this->fetch();
    }

    //会员每月业绩
    public function monthly_bonus(){
        $z_monthly_bonus = M('z_monthly_bonus');
        $smsTpls = $z_monthly_bonus->alias('b')->field('b.*,u.mobile')->join('users u','b.user_id = u.user_id')->order('year DESC,month asc')->select();
        $this->assign('smsTplList',$smsTpls);
        return $this->fetch();
    }
}