...
class System extends Base
{
	...
	public function index()
	{
        ...
        // dump($inc_type);
        // ,'order_advertising_news','bonus_notes','bonus_bonus','promotion_award','straight_award','peer_award'
...
//		tpCache($param['inc_type'],$param); //注释掉，不注释会出现重复写入数据库
    ...
    public function sales_award(){
       ...
       $sales_award = $z_sales_award->order("achievement asc")->select();
       $this->assign('sales_award',$sales_award);
       return $this->fetch();
    }


    public function add_sales_award(){
        $z_sales_award = M("z_sales_award");
        if(IS_POST){
            $dataPost = I('post.');
            $data = [
                'achievement' => $dataPost['achievement']*10000,
                'royalty_ratio' => $dataPost['royalty_ratio'],
            ];
            if(empty($dataPost['id'])){
                $res = $z_sales_award->add($data);
            }else{
                $res = $z_sales_award->where(['id'=>$dataPost['id']])->save($data);
            }
            if($res){
                $return['status'] = 1;
                $return['url'] = U('Admin/System/sales_award');
                $return['msg'] = "操作成功!!!";
            }else{
                $return['status'] = 0;
                $return['url'] = U('Admin/System/add_sales_award');
                $return['msg'] = "操作失败!!!";
            }
            ...
            $id = I('get.id');
            if(!empty($id)){
                $sales_award = $z_sales_award->where("id=".$id)->find();
                $this->assign('sales_award',$sales_award);
            }
            return $this->fetch();
        }
    ...
    public function del_sales_award(){
        $id = I('get.id');
        $z_sales_award = M("z_sales_award");
        $res = $z_sales_award->where("id=".$id)->delete();
        if($res){
            $msg = "操作成功!!!";
        }else{
            $msg = "操作失败!!!";
        }
        $this->success($msg,U('Admin/System/sales_award'));
    }
